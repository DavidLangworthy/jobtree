...
(*                                                                          *)
(* The full universally-quantified seeded-fold theorem is still too large    *)
(* for Apalache on the current bounded model. The checked obligation here is *)
(* the representative version the implementation actually needs on this      *)
(* history shape: the base settlement step `0 -> 1` and the incremental step *)
(* `1 -> 2`. Those two steps cover owned and borrowed carry-forward, the     *)
(* shared and env-local aggregate buckets, and lender-hour seeding.          *)
(*                                                                          *)
