...
| P6 | **An SMT monotonicity oracle for the funding ledger.** Motivated by the cross-vendor panel's HIGH finding at `pkg/funding/evaluate.go:661`, where `cl.tier = ev.Graph.Tier(acct.Owner, ev.OwnerOf(run.Namespace))` re-derives the owner **live** and applies it to the *entire* replayed lifetime. Note what the finding does **not** require: nothing moves. A Run cannot change namespace (`metadata.namespace` is immutable in Kubernetes — you delete and recreate, yielding a different object) and neither can a lease (`GPULeaseSpec is immutable after creation`, and `Spec.RunRef.Namespace` is pinned at mint). What is mutable is the namespace→owner **mapping**: add a second-owner Budget and `OwnerOf("ns-a")` answers differently than it did four hours ago, so a replay re-tiers history that already happened. The proposal is a per-PR gate asserting a topology change cannot retroactively re-class already-elapsed GPU-hours. (P5 above numbers itself; this is P6 to avoid colliding with it.) | **Which monotonicity property is the invariant?** Three candidates, not equivalent: **(1) charges never decrease** — flatly false on purpose if refunds or legitimate re-parenting exist; **(2) hours already elapsed never change classification** — the narrow reading, making accrued history immutable; **(3) total charged across all envelopes is conserved** under a topology change — a charge may move between envelopes but never vanish. Also: does a re-parented run's history follow it, or stay with the envelope that paid at the time? | Picking among these **defines what the ledger means** — specifically whether an admin action can ever be retroactive. It is a statement about funding semantics, not a coding choice, and it cannot be inferred from the code: the code's current behaviour is precisely what is in question. An oracle built on the wrong invariant is *confidently* wrong, which on the sole-committer / funding path is worse than no oracle. **Sequencing caveat:** the motivating finding is **traced, not reproduced** — Codex ran sandboxed read-only and compiled nothing — so if the panel's reproduction skeptic cannot reproduce it, this becomes a regression gate rather than a bug fix, which is a different value proposition. **Why SMT and not TLA+:** the claim is two-state (∀ state `S`, ∀ admissible mutation `m`), not temporal — no interleavings, no fairness, no state-space explosion — and `Evaluate`'s replay is close to a pure fold over `(leases, budgets, graph) → classes`, which is the best case for a solver. The cost hides in encoding `Graph.Tier`'s ancestry over the owner DAG; bounded owner-tree depth keeps it decidable, unbounded reachability drags it toward fixpoints and the easy win evaporates. |
