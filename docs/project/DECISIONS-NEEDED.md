...
| P5 | **R7 pt2 — the interior-tier injectivity exemption** (`pkg/funding/evaluate.go:220`). The paused adversarial review (`reviews/2026-07-24-r7-pt2-owner-from-namespace-f52d3cf/`) **confirmed by reproduction** that an owner which is BOTH directly leaf-bound in ≥2 runnable namespaces AND named as some `Budget.Spec.Parents` entry evades `ConflictLeafOwnerSpansNamespaces` → a Run in tenant-a mints a non-recallable **Owned** charge against tenant-b (F7's exact "confirm the interior-tier exemption cannot be abused" question, answered: it can). | **Narrow the exemption, or keep it and rely on RBAC + R26 alarm 3?** The R7 design (`R7-tenancy-amendment.md` §4/§5, C-4) *intentionally* exempts interior tiers (*"interior tiers may span admin namespaces; nothing classes Owned against them"*) and books the residual to the RBAC precondition + R26 alarm 3 — both posture / not-yet-implemented, i.e. **detection, not the prevention the injectivity fail-safe gives leaf owners.** | The design deliberately chose the exemption; the finding shows its leaf/interior dichotomy is unsound when one owner holds both roles. Narrowing it risks breaking the legitimately-allowed multi-namespace-pool case (a **reaper**), and the review's reaper-veto lens never ran. Changing a ratified tenancy invariant's guarantees is David's call, not an implementation detail. **Not guessed; working tree left untouched.** |
