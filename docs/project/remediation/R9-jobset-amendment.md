...
| Phase | Content | Size |
|---|---|---|
| **9A-0** | Ratify this amendment; delete `pkg/lowering` + retire JOBSET-3/4/5 and JOBSET-9/10's JobSet-creates-pods framing; move the mapping contract's text onto the emit path docs; fix the false rendezvous comments (`api/v1/run_types.go:47,67` — this is R10); update `hack/e2e/versions.env` note. No behavior change. | **S** |
| **9A-1** | Stable rendezvous identity: per-run headless Service (created/GC'd via Run ownerRef); `hostname`/`subdomain` on emitted pods; swap pod inherits the replaced member's ordinal name/hostname (replaces the `unixnano` name, `run_controller.go:1913`). Touches emit + bridge + swap; adversarial review (swap is sole-committer path). | **M** |
