...
| **9A-2** ✅ *(2026-07-10)* | Rendezvous env for a FIXED-width gang (`buildPod`→`injectRendezvousEnv`): `MASTER_ADDR=<run>-active-0.<svc>.<ns>.svc`, `MASTER_PORT=29500`, `WORLD_SIZE`, `NNODES`, `NODE_RANK` — all derived from the pod's ordinal hostname + the run's shape, so correct on every mint path (initial/top-up/swap) without per-path stamping; a swap inherits its rank via the 9A-1 hostname. `RANK`/`LOCAL_RANK` omitted (per-process, torchrun's). None injected for width==1, spares, or MALLEABLE runs (a static WORLD_SIZE is wrong for a resizing run — elastic rendezvous is separate). `Run.ValidateCreate` rejects researcher-set reserved env names; `buildPod` also overrides any that slip through. Unit + full envtest green. | **M** |
| **9A-3** ✅ *(2026-07-10)* | The failure edge (absorbs R8): `RunRole.FailurePolicy` (`Fail` default / `Retry(n[,backoff])` / `Ignore`) + `Retries`/`Backoff`, `status.failedAttempts`/`retryAfter`, webhook validation, CRD regen. `handleWorkloadFailure` (run_controller.go): Fail → `RunPhaseFailed` + `releaseRun("WorkloadFailed")` + Warning; Retry → close the failed member's lease, drop it, re-emit via top-up, count the attempt (backoff-gated), then Fail when exhausted; Ignore → `runGangComplete` counts a Failed member as terminal. Wired before the completion gate. PodFailed added to the pod-watch Create/Update predicate. Pure-engine tests cover all four paths; full envtest green. The LIVE failure proof (`failure-smoke.sh`, real exit 1) is 9A-4 — the antifake terminal-phase allowlist is capped at 2 and shrink-only, so no hand-injected envtest is added here (by design). | **L** |
| **9A-4** ✅ *(2026-07-10)* | Live proof on real kind clusters, standalone runnable like `swap-smoke.sh`. `rendezvous-smoke.sh`: a **2-node** kind cluster (1 GPU/node forces the two ranks apart), a width-2 gang, and a busybox `nc` handshake over the injected `MASTER_ADDR` + the per-run headless DNS — rank 0 listens on `MASTER_PORT`, rank 1 dials `MASTER_ADDR:MASTER_PORT` until it connects; both exit 0 ⇒ Run **Completed**. A real `nc` handshake (not torch) is a faithful proof of exactly the 9A-1 DNS + 9A-2 env plumbing: if either were wrong rank 1 could never reach rank 0, and the run would never complete. `failure-smoke.sh`: a real container `exit 1` on a real kubelet ⇒ Pod Failed ⇒ Run **Failed** ⇒ leases closed `WorkloadFailed` (pre-R8 it hangs; the antifake terminal-phase allowlist is capped-at-2/shrink-only, so a real kubelet — not a hand-set phase — must carry this proof). **Both ran green live (kind v1.36.1).** R5 forgery re-verify: the rendezvous fields (`hostname`/`subdomain`/env) are **manager-authored on the emit path** (`buildPod`), never researcher-supplied — `Run.ValidateCreate` rejects researcher-set reserved env names and `buildPod` overrides any that slip through — so they add no new forgery surface to the sole-committer path; satisfied by construction, no separate test. | **L** |
