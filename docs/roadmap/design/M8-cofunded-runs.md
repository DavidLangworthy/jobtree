...
- Surface reporting views showing per-run and per-owner cost splits via `Run.status.funding`.
...
- **Cover enhancements:** Extend `pkg/cover` to evaluate sponsor order after family sharing, enforcing lending sub-caps and per-run borrow limits.
- **Run controller updates:** Respect borrower guardrails when requesting cover plans; fail admission if borrowing would exceed `maxBorrowGPUs`; update status with funding summaries.
- **Status & metrics:** Record `Run.status.funding` (owned vs. borrowed GPUs/hours) and expose borrowed usage through the existing budget accounting metrics.
- **CLI additions:** `kubectl runs sponsors` to list/add/remove sponsors; `kubectl runs who-pays` to show split. (CLI scheduled for M9.)
...
- Controller tests verifying co-funded admission updates status and borrow-limit guardrails trigger reservations.
- Forecast unit tests covering borrow-limit messaging; e2e scenario planned alongside CLI work in M9.

## Observability & Telemetry
- Metrics: leverage budget controller metrics for borrowed concurrency/GPU-hours; add logs for borrowing decisions (envelope selected, reason) and guardrail violations.
