module github.com/davidlangworthy/jobtree
...
// PLUGIN-1: the out-of-tree scheduler binary (cmd/scheduler) depends on
// k8s.io/kubernetes for the kube-scheduler framework + app packages. The
// k8s.io/kubernetes module declares all of its staging libraries with the
// placeholder version v0.0.0 and pins them via replace directives to
// ./staging/src/... paths that do not exist outside its own repo. Consumers
// must therefore re-pin every staging module to the matching published tag
// (v0.36.2, aligned with k8s.io/api v0.36.2 already required above). This is
// the same replace block scheduler-plugins and Volcano carry. It only affects
// the scheduler build; the manager and CLI are unchanged.
replace (
